...
//When page load user click aotumatically active
userWord.focus();



function randWordApi(){
   const randwordCall = fetch(`https://random-words-api.vercel.app/word`)
   .then(res => res.json())
   .then(data => 
   
      {const randWord = data[0].word
         showRandWord(randWord);  
      });
               
}


function showRandWord(randWord){
      return randWord;   
   
}

console.log(showRandWord());
