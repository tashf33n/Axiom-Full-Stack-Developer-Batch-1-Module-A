...
function searchMeal(e){
    ...
    e.preventDefault();
    //Get the vale form search input field
    ...
    if (searchText.trim()){
        ...
        .then(data => {
            ...
            //Clear previous search meal info
...
function displayMealDetails(meal){
    ...
    for( let i = 1; i <= 20; i++){
        ...
        if (meal[`strIngredient${i}`]){
            //Push all the ingredient and measurements into the ingredients array
...
function randomMeal(){
    ...
    fetch(`https://www.themealdb.com/api/json/v1/1/random.php`)
        .then(res => res.json())
        .then(data => {
            const randomMealInfo = data.meals[0];
            console.log(randomMealInfo);
            displayRandomMealDetails(randomMealInfo);
        })
}

function displayRandomMealDetails(meal){
    //Clear Meals inner results
    meals.innerHTML = '';
    selectedMeal.innerHTML = '';
...
meals.addEventListener('click', e => {
    //Find and return only if clicked on the meal-info div 
...
//3. Listen for click on random button to get random meal
